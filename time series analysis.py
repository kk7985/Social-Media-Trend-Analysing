...
daily_posts = df.set_index('created_time').resample('D').size()

# Plot daily post counts
daily_posts.plot(title='Daily Posts Over Time')
plt.xlabel('Date')
plt.ylabel('Number of Posts')
plt.show()

# Decompose the time series to observe trends and seasonality
decomposition = sm.tsa.seasonal_decompose(daily_posts, model='additive')
decomposition.plot()
plt.show()
